  ...
  fs.readdir(Release.root, function(err, entries) {
    if (err) {
      ...
      async.map(entries, function(entry, done) {
        Release.createFromFile(path.join(Release.root, entry), done);
      }, callback);
    }
  });
};
 ...
 */
Release.createFromString = function(str) {
 ...
 *     error.
 */
Release.createFromFile = function(file, callback) {
  fs.readFile(file, function(err, data) {
    if (err) {
      ...
      var release;
      try {
        release = Release.createFromString(String(data));
      } catch (err) {
        callback(err);
        return;
      }
