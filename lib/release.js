...
/**
 * Get the path to the symbols file for the release.
 * @param {string} name Release name.
 * @return {string} Path to release symbols file.
 */
function getSymbolsFile(name) {
  return path.join(releaseRoot, name, 'build', 'symbols.json');
}
...
/**
 * Get the symbols metadata for the release.
 * @param {function(Error, Object)} callback Called with the parsed symbols.json
 *     file or any error.
 */
Release.prototype.getSymbols = function(callback) {
  var name = this.name;
  fs.readFile(getSymbolsFile(name), function(err, data) {
    if (err) {
      var msg = 'Failed to read symbols file for ' + name + ': ' + err.message;
      callback(new Error(msg));
    } else {
      var symbols;
      try {
        symbols = JSON.parse(String(data));
      } catch (err) {
        var msg = 'Error parsing symbols file for ' + name + ': ' + err.message;
        callback(new Error(msg));
        return;
      }
