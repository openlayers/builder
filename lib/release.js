...
function getInfoFile(name) {
  return path.join(releaseRoot, name, 'build', 'info.json');
}
...
Release.prototype.getInfo = function(callback) {
  ...
  fs.readFile(getInfoFile(name), function(err, data) {
    if (err) {
      var msg = 'Failed to read info file for ' + name + ': ' + err.message;
      ...
      try {
        ...
        var msg = 'Error parsing info file for ' + name + ': ' + err.message;
        callback(new Error(msg));
        return;
      }
      callback(null, info);
