  ...
  Release.findOne({name: request.params.release}, function(err, release) {
    ...
    if (release) {
      build.getIndex(release, function(err2, index) {
        ...
        if (request.query.longId) {
          build.getJobByLongId(release, request.query.longId, function(err3, job) {
            if (err3) {
              // TODO: check for invalid longId error
              reply(boom.internal('Failed to get build index'));
              return;
            }
            reply(job ? [job] : []);
...
exports.createJob = function(request, reply) {
  Release.findOne({name: request.params.release}, function(err, release) {
    ...
    if (release) {
      build.getOrCreateJob(release, request.payload, function(err2, job) {
        if (err2) {
